...
trait Select
{
    ...
    public static function select_channels( $options = [], $fieldname = 'name', $js = null, $required = false ): string
    {
        ...
        $select .= '</th>';
        $select .= '<td>';
        $select .= '<select onchange="' . $js_function . '" name="' . strtolower( str_replace( ' ', '_', $fieldname ) ) . '" id="' . strtolower( str_replace( ' ', '_', $fieldname ) ) . '" class="uk-select form-select">';
        // Options list Output.
        if ( \is_array( $options ) ) {
            foreach ( $options as $key => $value ) {
                asort( $value );
                $select .= '<option value="' . $value['uuid'] . '">' . ucfirst( $value['title'] ) . '</option>';
            }
        }
        $select .= '</select>';
